...
void outlineRefreshLine() {
  ...
  O.screencols + OUTLINE_RIGHT_MARGIN + OUTLINE_LEFT_MARGIN, O.cy+1, 1);
...
void outlineRefreshScreen() {
  ...
  for (int j=0; j < O.screenrows;j++) {
    ...
    O.screencols + OUTLINE_RIGHT_MARGIN + OUTLINE_LEFT_MARGIN); //erase from cursor to left
    abAppend(&ab, buf, strlen(buf));
  }
