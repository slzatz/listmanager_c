...
//the vim modes used both by titles (outline) and note (editor)
#define NORMAL_MODE 0
#define INSERT_MODE 1
#define COMMAND_LINE_MODE 2
#define VISUAL_LINE_MODE 3
#define VISUAL_MODE 4
#define REPLACE_MODE 5
...
void outlineRefreshScreen() {
  ...
  for (int j=0; j < O.screenrows;j++) {
    snprintf(buf, sizeof(buf), "\x1b[%d;%dH\x1b[1K", j, O.screencols + 3); //erase from cursor to left
    abAppend(&ab, buf, strlen(buf));
  }
 ...
 ***************************************/
  ...
  switch (c) {
    ...
    case 'r':
      O.mode = REPLACE_MODE; //REPLACE_MODE = 5
      return;
...
// for testing purposes I am using CTRL-h in normal mode
  ...
  } else if (O.mode == COMMAND_LINE_MODE) {
  ...
  } else if (O.mode == VISUAL_LINE_MODE) {
