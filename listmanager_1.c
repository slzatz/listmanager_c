...
void editorMoveCursor(int key) {
  ...
  switch (key) {
    case ARROW_LEFT:
    case 'h':
      // note E.cx might be zero but filecol positive because of E.coloff
      // then E.cx goes negative
      // dealt with in EditorScroll
      if (get_filecol() > 0) E.cx--; 
      break;
    ...
    case ARROW_UP:
    case 'k':
      // note E.cy might be zero but filerow positive because of E.rowoff
      // then E.cy goes negative
      // dealt with in EditorScroll
      if (get_filerow() > 0) E.cy--; 
      break;

    case ARROW_DOWN:
    case 'j':
      if (get_filerow() < E.numrows - 1) E.cy++;
      break;
  }
...
void initEditor() {
  ...
  E.rowoff = 0;  //number of rows scrolled off the screen
...
int main(void) {
 ...
 // PQfinish(conn);
  ...
  while (1) {
    int scroll = editorScroll();
    if (scroll) editorRefreshScreen(); 
    else editorRefreshLine();
